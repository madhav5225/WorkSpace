...
function sendMsg(event) {
    ...
    if (typeof messageList[conversation_id] == 'undefined' || messageList[conversation_id].length == 0)
        ...
        msgId = messageList[conversation_id][length - 1].msgId + 1;
    }
    if (msg != '') {
...
function setChat(x) {
    ...
    $.get('/msgList', { conversation_id }, function (message) {
        ...
        for (var i = 0; i < message.length; i++) {
            ...
            if (obj.sender == currentUser._id) {
                if (obj.isSeen == 0)
                    ($('<li class="right  message-box" id=msg' + obj.msgId + '>').
                        text(obj.msg)).appendTo('#chat_ul').append('<i class="material-icons">done_all</i>');
                else
                    ($('<li class="right  message-box" id=msg' + obj.msgId + '>').
                        text(obj.msg)).appendTo('#chat_ul').append('<i class="material-icons"style="font-size:48px;color:red">done_all</i>');
